...
export function BasicInfoForm() {
  ...
  const handleImageUpload = (e, imageType) => {
    ...
    try {
      if (imageType === 'extra_images') {
        ...
        const newExtraImages = [...formData.extra_images]
  ...
  const takePhoto = () => {
    ...
    canvas.toBlob(blob => {
      ...
      if (activeCamera === 'extra_images') {
        ...
        setFormData(prev => ({
          ...prev,
          extra_images: [...prev.extra_images, file]
        }))
        
        setImagePreviews(prev => ({
          ...prev,
          extra_images: [...prev.extra_images, previewUrl]
        }))
      } else {
  ...
  const handleSubmit = (e) => {
    ...
    const submitData = {
      ...
      price_description: formData.price_description === 'custom' ? formData.custom_price_description : formData.price_description,
      // File objects would typically be uploaded to a server or cloud storage
      main_image: formData.main_image ? formData.main_image.name : null,
      side_image1: formData.side_image1 ? formData.side_image1.name : null,
      side_image2: formData.side_image2 ? formData.side_image2.name : null,
      extra_images: formData.extra_images.map(img => img.name)
    }
    
    // Remove the custom_price_description field from the submission
    delete submitData.custom_price_description
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">Property Basic Information</h2>
      <p className="text-gray-600 mb-6">
        Tell guests about your property and what makes it special.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {formData.price_description === 'custom' && (
                  <input
                    type="text"
                    id="custom_price_description"
                    name="custom_price_description"
                    value={formData.custom_price_description}
                    onChange={handleInputChange}
                    placeholder="E.g., for 3 days, for weekend..."
                    className="block w-full rounded-lg border border-gray-200 px-4 py-3 focus:border-[var(--primary-red)] focus:outline-none focus:ring-1 focus:ring-[var(--primary-red)]"
                    required={formData.price_description === 'custom'}
                  />
                )}
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          {/* Main and Side Images Layout */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Main Image (2/3 width on desktop) */}
            ...
            {/* Side Images (1/3 width on desktop, stacked vertically) */}
            <div className="md:col-span-1 space-y-4">
              ...
              <div>
                ...
                <div className={`
                  relative flex flex-col items-center justify-center h-28 rounded-lg border-2 border-dashed
